...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 23 15:39:36 2021
...
# final_img, background = subtract_background_rolling_ball(img, radius, light_background=True, use_paraboloid=True, do_presmooth=False)


def preSegmentGradBlur(img):
    ...
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT,  
                                   filterSize) 
    gradient = cv2.morphologyEx(clahe_img, cv2.MORPH_GRADIENT, kernel)
    gradient_blur = cv2.medianBlur(gradient,5)
    return gradient_blur

def preSegmentGrad(img):
    ##### Contrast limited adaptive histogram equalization (CLAHE) 
    clahe = cv2.createCLAHE(clipLimit=3, tileGridSize=(8,8))
    clahe_img = clahe.apply(img)
    filterSize =(5, 5) 
...
def generateLabels(gradient):
    kernel_convolve = np.ones((5,5),np.float32)/25
    dst = cv2.filter2D(gradient,-1,kernel_convolve)
...
def imgregionProps(img,labels,area,imgName):
    ...
    style = dict(size=20, color= 'yellow') #Style for the text to be added to the image
